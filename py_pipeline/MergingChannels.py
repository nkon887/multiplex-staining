...
class MergingChannels:
    ...
    def get_channel_files(self, subfolder, marker):
        ...
        for subfolder_file in os.listdir(subfolder):
            pattern = r'^\d{6}[^{\.}]*'
            if marker in os.path.basename(subfolder_file) and re.match(pattern + r'.tif',
                                                                       os.path.basename(subfolder_file)):
                channel_files.append(subfolder_file)
        return channel_files

    def process(self, dapi_file, selected_channel_files, output_dir, force_save):
...
def main():
    ...
    markers = []
    dapi_files_dict = {}
    merging_channels = MergingChannels()
    ...
    for subfolder in subfolders:
        ...
        markers = markers + merging_channels.get_channels(subfolder, config.dapi_str)
        if not dapi_files:
    ...
    try:
        selected_markers, force_save = merging_channels.getting_input_parameters(dapi_files_dict, markers)
    except:
    ...
    for subfolder in subfolders:
        ...
        for marker in markers:
            if selected_markers.get(marker) and marker != config.dapi_str + subfolder:
                selected_channel_files = selected_channel_files + merging_channels.get_channel_files(subfolder, marker)
        ...
        merging_channels.process(selected_dapi_image_path, selected_channel_files_paths, output_dir, force_save)
