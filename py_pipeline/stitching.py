...
class stitchingTools:
    ...
    def save_singleplanes(self, imp, savepath, metainfo, format='tiff'):
        ...
        for s in range(1, stack.size() + 1):
            slicetitle = metainfo["fileID"] + "_" + "c" + str(
                s - 1) + "." + format  # + metainfo["channel " + str(s)] + "." + format
            stackindex = s
    ...
    def write_txt(self, metainfo, savingDir):
    ...
    def process(self):
        ...
        for image_file in os.listdir(dir):
            if image_file.endswith(".czi") and (re.search(".*shading.*", image_file)):
                shadingfile = os.path.join(dir, image_file)
        for image_file in os.listdir(dir):
            IJ.log(str(image_file))
            ...
            if image_file.endswith(".czi") and not (re.search(".*shading.*", image_file)):
                ...
                # export the ImgPlus
                savepath1 = os.path.join(savingDir, r"LOCI.ome.xml")
                paramstring = "outfile=[" + savepath1 + "] windowless=true compression=Uncompressed saveROI=false"
                plugin = LociExporter()
                ...
                t1 = time.time()
                total1 = t1 - t0
                print("Export complete OME-TIFF using LOCI: {}".format(total1))
                for i, imp in enumerate(imps):
                    imp3 = ImageCalculator().run("Subtract create stack", imp, shadefile[0])

                    tile_name = "tile_" + str(i + 1).zfill(3) + ".tif"
                    IJ.log("Saving " + tile_name + " under " + savingDir)
                    HyperStackConverter.toStack(imp3)
                    FileSaver(imp3).saveAsTiff(os.path.join(savingDir, tile_name))
        ...
        # Save the log file
        IJ.selectWindow("Log")
        thisFile = os.path.join(dir, "CZI_to_TIF.log")
        IJ.saveAs("Log", thisFile)